...
import { getOrderPenddingByUser, getOrderShippingByUser } from '../../../redux/actions/OrderAction'
    ...
    return (
        <div className="myorder-menu">
            ...
            >
                ...
                {myOrdersPendding ? <div className="myorder-menu-item-newPendding">{myOrdersPendding.length}</div> : ''}
            </div>
            <div
                className={location.pathname === '/myOrder/shipping' ? 'myorder-menu-item active' : 'myorder-menu-item'}
            >
                <Link to="/myOrder/shipping">Đang giao</Link>
                {myOrdersShipping ? <div className="myorder-menu-item-newShipping">{myOrdersShipping.length}</div> : ''}
            </div>
